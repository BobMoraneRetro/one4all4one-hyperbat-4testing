...
uniform COMPAT_PRECISION vec4 glowColor; // Couleur du halo (RGB ; alpha ignoré). En export ES c'est une couleur HEX → ES ne sait PAS lire un vec4 en floats, mais sait lire une couleur hex.
uniform COMPAT_PRECISION float glowIntensity; // Intensité (10..100) — uniforme FLOAT séparé (ES lit les floats simples ; l'intensité ne peut donc pas vivre dans glowColor.a)
...
void main(void)
{
    ...
    // Couleur = glowColor.rgb ; intensité = uniforme glowIntensity (FLOAT séparé,
    // car ES ne sait pas passer un vec4 en floats → on passe la couleur en HEX et
    // l'intensité à part). intensité 0 → défaut blanc (glow activé sans réglage).
    vec3 glowRGB = glowColor.rgb;
    float intensity = glowIntensity;
    if (intensity == 0.0 && glowSize != 0.0) { glowRGB = vec3(1.0); intensity = 20.0; }
    ...
    vec4 finalGlow = vec4(glowRGB, glowA);
