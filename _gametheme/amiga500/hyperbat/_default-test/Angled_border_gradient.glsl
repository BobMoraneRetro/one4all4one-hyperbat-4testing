...
void main(void)
{
    ...
    float tr = max(gradientTransition, 0.001); 
    ...
    if (!outside)
    {
        ...
        // On récupère la couleur de la texture SANS multiplier par v_col ici
        vec4 texColor = COMPAT_TEXTURE(u_tex, distorted);
        // On mixe l'image et la bordure
        FragColor = mix(texColor, borderCol, alphaInner);
        // ON APPLIQUE L'OPACITÉ DU XML À LA TOUTE FIN
        FragColor *= v_col;
    }
    else
    {
        // Pour la partie extérieure (bordure externe), on applique aussi v_col
        FragColor = vec4(borderCol.rgb, borderCol.a * outsideAlpha) * v_col;
